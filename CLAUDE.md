...
src/
  ...
  validations/
    rainlog.validations.ts    # express-validator chains for rainlog POST body
  utils/
...
- **Router:** `src/routes/rainloggerRouter.ts` defines the `/rainlog` route. Currently supports `POST` (create a rainlog entry). Non-allowed methods are rejected with a 405 via the `methodNotAllowed` utility.
...
- Implement remaining CRUD routes (GET, PUT, DELETE) in `rainloggerRouter.ts`.
